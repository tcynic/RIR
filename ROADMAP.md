...
This document captures follow-on work for the Release Intent Records (RIR) repository beyond the v1 scaffold. Items are grouped by what they unblock, not by delivery date. Pick from the top of each section first; deeper items earn their place as the practice matures.
...
Items that make the next team's first intent easier to write and reduce the gap between scaffold and lived practice.
...
The hardest intent type to declare cleanly because the user is another engineer or team rather than an end customer. Validating the template against a Platform/Enablement release confirms the framework's claim that the same artifact serves all six types. Produces an additional `examples/RI-NNNN-...md` in the same evaluated state as the Growth example.

### Worked examples: the remaining four intent types
...
### New-intent helper script
A small `scripts/new-intent.sh` that finds the next sequential number, prompts for a slug, copies `templates/minimal.md` to the right location, and opens the file. Removes a small but real friction at the start of writing an intent. Should not enforce additional rules; CI is the enforcement layer.

### Service-repo PR template
A short markdown template that service repositories can copy into `.github/pull_request_template.md` requiring an intent ID reference. Strengthens the cross-repo linkage convention by surfacing the omission at PR-open time rather than at review.

## Reporting

Items that become valuable once the repo accumulates more than a handful of intents.

### Generated index page
A static index, rebuilt on merge to main, listing every intent with its status, type, owners, declared and evaluated dates, and a link to the file. Unblocks "find me every Growth intent from Q3" without grepping. Could ship as a GitHub Pages site, a generated `INDEX.md`, or both.
...
### Stale-intent report
Identify intents that have been `proposed` for more than a configured threshold, or `active` past their Horizon. Both states represent process failures the framework cares about; the report is the visibility mechanism that turns implicit drift into explicit work.

### Maturity ladder rollup
Once the repo has enough evaluated intents, compute the team's position on the R.I.V.E.R. maturity ladder (Deploy, Control, Declare, Prove, Learn) from the artifact corpus. Genuinely speculative as a v2 item because the ladder's empirical calibration is still ahead of the framework. Worth keeping on the list as a marker.
...
The intent declares the *planned* progression (5%, 25%, 50%, 100%); the flag platform's audit log holds the *actual* progression (when each stage transitioned, which guardrails fired). A Timeline section captured at evaluation time would put both in the document. Useful but not yet missed.
...
A linter that estimates whether the declared cohort size is large enough to detect the declared magnitude at conventional statistical power. Catches "20% lift on a cohort of 80" before the intent is merged. Requires modeling assumptions the framework does not yet specify; deferrable.
...
### Co-locating intents with service code
Considered and rejected. Release Intents are cross-functional product artifacts, not engineering documentation, and frequently span repositories. Linkage is via intent ID, not filesystem adjacency.
...
### Pairing each intent with a separate evaluation file
